...
async def stream_agent(
    ...
    if not body.thread_id:
        print("Missing thread_id field")
        raise HTTPException(status_code=422, detail="Missing required field: thread_id")
    ...
    # Validate agent_type
    if body.agent_type not in ["fast", "deep"]:
        print(f"Invalid agent_type: {body.agent_type}")
        raise HTTPException(status_code=422, detail="Invalid agent_type. Must be 'fast' or 'deep'")
    
    # Validate Authorization header
    if not api_key:
        print("Missing Authorization header")
        raise HTTPException(status_code=422, detail="Missing Authorization header. Please provide a valid Tavily API key.")
